...
public class SpatialIndexes {
	...
	private static class GridIndexBuilder<T extends Nodal> implements SpatialIndexBuilder<T> {
		...
		public boolean isWithinBounds(T obj){
		...
		public void insert(T obj) {
			 ...
			 List<T> cell = (List<T>)this.grid[xCellIdx][yCellIdx];
			 if ( cell == null ) {
		...
		private Object[][][] toCompressedArray(List<T>[][] grid) {
	...
	private static class GridIndex <T extends Nodal> implements SpatialIndex<T> {
		...
		public List<T> getNClosest(Nodal loc, int num, int maxDistance) {
			...
			for (int i = 0 ; i < Math.min(num, candidates.size()); i++){
				result.add((T)candidates.get(i).obj);
			}
			
			return result;
		}

		public List<T> query(Envelope window) {
